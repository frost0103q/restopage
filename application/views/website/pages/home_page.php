<div id="wrapper">
    <div class="slider-wrapper theme-default">
        <?php 
            ...
            if (isset($sliders[0]) && $sliders[0]->image_name !== "" ){ ?>
                <div id="wowslider-container1" class="wowslider">
                    <style>
                        .ws_cover{
                            background: <?=$page_contents->slider_overlay_color?> !important;
                            opacity:<?= $page_contents->slider_overlay_alpha?> !important;
                        }
                    </style>
                    <div class="ws_images">
                        ...
                        <ul>
                            <li>
                                ...
                                <img src="<?= base_url("assets/home_slider_images/").$sliders[0]->image_name?>" width ="100%" alt="<h3><?=$sliders[0]->$slider_caption_title_field == "" ? $sliders[0]->slider_caption_title : $sliders[0]->$slider_caption_title_field ?></h3><p><?=$sliders[0]->$slider_caption_content_field == "" ? $sliders[0]->slider_caption_content : $sliders[0]->$slider_caption_content_field?></p>" title="<h3><?=$sliders[0]->$slider_caption_title_field == "" ? $sliders[0]->slider_caption_title : $sliders[0]->$slider_caption_title_field ?></h3><p><?=$sliders[0]->$slider_caption_content_field == "" ? $sliders[0]->slider_caption_content : $sliders[0]->$slider_caption_content_field?></p>" id="wows1_0" />
                            </li>
                        </ul>
                    </div>
                </div>
            <?php }else{ ?>
                <div class="d-flex justify-content-center align-items-center p-4 slider-welcome-section">
                    <h4 id="slider-welcome-section"></h4>
                </div>
            <?php }
        ?>
    </div>
    <div class="jc-home-order-confirm text-center d-flex justify-content-center p-md-4 p-2">
        <div class="jc-home-order-confirm-wrap">
            <?php if($myRestDetail->dp_option % 4  > 0 && $myRestDetail->resto_plan == "pro"){ ?>
                <p class="jc-home_view_text"><?= $this->lang->line("Your Order will be confirmed in REAL-TIME")?></p>
            <?php }?>
            <div class="d-flex justify-content-around align-items-center">
                <?php if($myRestDetail->dp_option > 0 && $myRestDetail->resto_plan == "pro"){ ?>
                    <?php if($myRestDetail->dp_option == 1 ||  $myRestDetail->dp_option == 3){  ?>
                        <span class="j-delivery_icon j-order_icon">
                    ...
                    <?php } ?>
                    <?php if($myRestDetail->dp_option == 2){  ?>
                        <p class="jc-home_view_text"><?= $this->lang->line("Your Order will be confirmed in REAL-TIME")?></p>
                        <span class="j-takeaway_icon j-order_icon">
                            <svg version="1.1" id="Ebene_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                    ...
                    <?php } ?>
                <?php }?>
                <a class="btn j-orange-bg jc-home_view_menubtn text-uppercase" href = "<?= base_url("view/").$rest_url_slug?>"><?= $this->lang->line("See")?> <?= $this->lang->line("Menu")?> <?php if($myRestDetail->dp_option % 4 > 0 && $myRestDetail->resto_plan == "pro"){ ?> & <?= $this->lang->line("Order")?> <?php } ?></a>
                <?php if($myRestDetail->dp_option > 0 && $myRestDetail->resto_plan == "pro"){ ?>    
                    <?php if($myRestDetail->dp_option == 1){  ?>
                        <span class="j-delivery_icon j-order_icon">
                            <svg version="1.1" id="Ebene_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                viewBox="0 0 233.33 158.16" style="enable-background:new 0 0 233.33 158.16;" xml:space="preserve">
                            <g>
    ...
    <?php if ($page_contents && $page_contents->is_show_welcome){ ?>
        <div class="container py-5">
            <div class="row section-I">
                <?php
                    if (isset($page_contents)){
                        if ($page_contents->content_image !== ""){ ?>
                            <div class="col-md-4">
                                <img class="w-100" src= '<?= base_url("assets/home_images/").$page_contents->content_image ?>' />
                            </div>
                            <?php }
                        }
                    ?>
                <div class="col">
                    <?php
                        $content_field = "content_".$site_lang;
                        if (isset($page_contents->content) && isset($site_lang)){
                            if ($page_contents->$content_field !== ""){
                                echo $page_contents->$content_field;
                            }else{
                                echo $page_contents->content;
                            }
                        }else{
    ...
    <?php } ?>
    <?php if ($page_contents && $page_contents->is_show_service && sizeof($homepage_services) > 0){ ?>
        <div class="row section-II mx-0" <?= isset($page_contents) ? 'style="background-image:url('.base_url("assets/home_service_background/").$page_contents->service_section_background.');"' : '' ?>>
            <div class="col-md-12 p-md-5 text-center text-uppercase">
                <?php if (isset($page_contents)){ 
                    $service_top_subject_field = "service_top_subject_".$site_lang;
                    $service_main_subject_field = "service_main_subject_".$site_lang;
                    $service_description_field = "service_description_".$site_lang;
                    ?>
                    <p class="mb-0 mt-4 jc-service"><?= isset($page_contents->$service_top_subject_field) && $page_contents->$service_top_subject_field == "" ? $page_contents->service_top_subject : $page_contents->$service_top_subject_field ?></p>
                    <h3 class="h3 jc-service"><?= isset($page_contents->$service_main_subject_field) && $page_contents->$service_main_subject_field == "" ? $page_contents->service_main_subject : $page_contents->$service_main_subject_field  ?></h3>
                <?php } ?>
                <div class="d-flex justify-content-center flex-row flex-wrap">
                    <?php 
                        ...
                        foreach ($homepage_services as $hs_key => $hs_value) { 
                            $hs_index ++; 
                            if ($hs_value->service_status == 1){
                            ?>
                            <div class="mt-3 homepage-service-item jc-servicebtn">
                            <!-- <div class="mt-3 homepage-service-item jc-servicebtn col-md-2"> -->
                ...
                <?php if (isset($page_contents)){ ?>
                    <p class="mb-0 desc jc-service"><?= $page_contents->$service_description_field == "" ? $page_contents->service_description : $page_contents->$service_description_field  ?></p>
                <?php } ?>
            </div>
        </div>
    <?php } ?>
