		...
		<div class="card-body">
            <div class="row">
                <div class="col-md-12 d-flex justify-content-between align-items-center mb-4">
                    <h1 class="h4 text-gray-900 text-capitalize"> Client #<?= $customer_id?> <?= $filter_type?> <?= $this->lang->line('Orders Management')?></h1>
                    <div class="order-navigation d-flex">                    
                        <div class="j-status-badge accepted-badge"> 
                            <span>
                                <a class="collapse-item" href="<?=base_url('Restaurant/clientOrder/').$customer_id."/accepted" ?>"><?= $this->lang->line('Accepted')?></a> 
                            </span>
                        </div>
                        <div class="j-status-badge pending-badge"> 
                            <span>
                                <a class="collapse-item" href="<?=base_url('Restaurant/clientOrder/').$customer_id."/pending" ?>"><?= $this->lang->line('Pending')?></a> 
                            </span>
                        </div>
                        <div class="j-status-badge canceled-badge"> 
                            <span>
                                <a class="collapse-item" href="<?=base_url('Restaurant/clientOrder/').$customer_id."/rejected" ?>"><?= $this->lang->line('Rejected')?></a> 
                            </span>
                        </div>
                        <div class="j-status-badge finished-badge"> 
                            <span>
                                <a class="collapse-item" href="<?=base_url('Restaurant/clientOrder/').$customer_id."/finished" ?>"><?= $this->lang->line('Finished')?></a> 
                            </span>
                        </div>
                        <div class="j-status-badge all-badge"> 
                            <span>
                                <a class="collapse-item" href="<?=base_url('Restaurant/clientOrder/').$customer_id."/all" ?>"><?= $this->lang->line('All Orders')?></a>
                            </span>
                        </div>
                    </div>
                </div>
                        ...
                        <?php
                            ...
                            foreach ($orders as $okey => $ovalue) { ?>
                                <tr class="order-row order-row-<?=$i?>  <?=$ovalue->order_type?>-row" data-id = "<?= $i?>">
                                    ...
                                    <td class="text-center align-middle order-date"><?= date("d.m.Y | H:i:s", strtotime($ovalue->order_date))?></td>
                                    <td class="text-center align-middle"><strong><?=$currentRestCurrencySymbol?><?= $ovalue->order_amount?></strong><br><span class="text-uppercase"><?= $ovalue->order_payment_method?></span></td>
                                    <td class="text-center align-middle text-uppercase"><?= $ovalue->order_type?></td>
                                    ...
                                    <td class="text-center align-middle">
                                        <select class="order_status form-control" data-status = "<?= $ovalue->order_status ?>" d-order_id = <?= $ovalue->order_id?> d-payment_method = <?= $ovalue->order_payment_method?> d-order_type ="<?= $ovalue->order_type?>">
                                            ...
                                            <option value="finished" <?= $ovalue->order_status =="finished" ? "selected" : "" ?> >Finished</option>
